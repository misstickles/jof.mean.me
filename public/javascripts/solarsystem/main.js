; function Solar3D(opts) {
	...
	opts.star_field_visible = opts.star_field_visible || true;
	opts.constellation_lines_visible = opts.constellation_lines_visible || true;
	...
	var stats, scene, renderer,
		...
		mouse, raycaster,
		milkyWay = null,
		stars, starIdLocation = {},
		jed = toJED(new Date()),
		clock = new THREE.Clock(),
		starField, constellationLines,
		gradientRamp,
	...
	$.getJSON('http://localhost:3000/api/solarsystem/stars', function(data) {
		loadImage(opts.static_prefix + '/star_ramp.png',
			function() {
				...
				$.getJSON('http://localhost:3000/api/solarsystem/constellations', function(data) {
					//constellationLines = generateConstellations(data);
					//constellationLines.visible = opts.constellation_lines_visible;
	...
	function initGUI() {
		var guiControls = function() {
			...
			this['Show Constellations'] = opts.constellation_lines_visible;
		...
		window.onload = function() {
			...
			gui.add(text, 'Show Constellations').onChange(function() {
...
//        light.shadowDarkness    = 0.2;
	...
	function toggleConstellationLines() {
		constellationLines.visible = opts.constellation_lines_visible = !opts.constellation_lines_visible;
	}
...
//            sizeAttenuation: true,
		...
		stars.forEach(function(star, idx) {
			...
			var i = idx * 3;
			positions[i + 0] = star[6] * radius;
			positions[i + 1] = star[7] * radius;
			positions[i + 2] = star[8] * radius;

			// TODO: here we are saving ids vs location for constellations
			// think how to do this more efficiently...
			if (star[1] !== 0) {
				starIdLocation[star[1]] = [positions[i + 0],positions[i + 1],positions[i + 2]];
			}

			var hueShift = 1.0 - star[9];
	...
	function generateConstellations(data) {
		var material = new THREE.LineBasicMaterial({
			color: 0xffff00
		});

		$.each(data, function(idx, itm) {
			// data[0] is from, there can be many to...
			// TODO: mouse over / label with name
			$.each(itm.data, function(idx, id) {
				var from = starIdLocation[id[0]];

				$.each(id, function(idx, id) {
					if (idx === 0) return;

					var to = starIdLocation[id];
...
//            console.log(idx + ' ' + itm.name + ' ' + itm.data[0]);
		});

		// var geometry = new THREE.Geometry();
		// geometry.vertices = new THREE.Vector3(10, 100, 50);
		// var line = new THREE.Line(geometry, material);
